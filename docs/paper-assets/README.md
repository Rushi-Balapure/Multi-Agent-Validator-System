...
- [`tables/`](tables/) — result-table stubs. [b2_live_vs_mock](tables/b2_live_vs_mock.md) compares B2 live vs dry-run mock and is not a measured result. Cells cite [F-native-scifact](formulas/F-native-scifact.md) and [F-false-endorsement](formulas/F-false-endorsement.md).
...
Nothing here is a result. Figures and formula notes are added with the locked experiments described in [the implementation plan](../IMPLEMENTATION_PLAN.md). The research scope stays the [two-month research plan](../../Validator_Agent_Two_Month_Research_Plan.pdf).
