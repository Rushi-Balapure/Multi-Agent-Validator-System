...
- The comparison table is [v_vs_b2_false_endorsement.md](../tables/v_vs_b2_false_endorsement.md). The B2 column cites the live development run `same-evidence-b2-development-s0-n20-dac855c4e2ae`. The V column cites Phase-3 gather live `validator-v-gather-development-s0-n20-90129d9056fd` on the same 20 claim ids. Every gather row is `execution_status=failed` (fail-closed D0), so `n_skipped_not_ok=20`, `n_scored=0`, and FE rates are **N/A** (not 0.0). The offline fixture-report dry-run under `tables/validator_v/` remains available via `--paper-table-fixture-v`. `src/evaluation/bootstrap.py` stores the 2000-resample plan and does not emit an interval until paired same-claim-id V and B2 predictions are both scorable.
