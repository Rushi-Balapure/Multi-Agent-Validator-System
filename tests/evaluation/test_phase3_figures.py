...
V_GATHER_RUN = "validator-v-gather-development-s0-n20-90129d9056fd"
...
def test_phase3_figures_cite_saved_run_ids(tmp_path: Path):
    ...
    assert fe_meta["v_run_id"] == V_GATHER_RUN
    assert fe_meta["v_fe_rates"] is None
    assert fe_meta["v_n_skipped_not_ok"] == 20
    ...
    assert hist["counts_by_label"] == {"NEI": 20}
    status = json.loads(
        (out / "v_gather_fail_closed_status.meta.json").read_text(encoding="utf-8")
    )
    assert status["counts_by_execution_status"] == {"failed": 20}


def test_checked_in_graphs_meta_matches_gather_run():
    meta = json.loads((GRAPHS / "fe_bar_v_vs_b2.meta.json").read_text(encoding="utf-8"))
    assert meta["v_run_id"] == V_GATHER_RUN
    assert meta["b2_run_id"] == B2_RUN
    assert meta["v_fe_rates"] is None
